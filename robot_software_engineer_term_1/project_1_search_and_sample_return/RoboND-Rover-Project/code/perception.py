...
def rock_scan(img, rgb_thresh=(110, 110, 50)):
    rock_detect = np.zeros_like(img[:,:,0])

    yellow_range = (rgb_thresh[0] < img[:,:,0]) \
                & (rgb_thresh[1] < img[:,:,1]) \
                & (rgb_thresh[2] > img[:,:,2])
    
    rock_detect[yellow_range] = 1
...
def perception_step(Rover):
    ...
    obs_x_pix, obs_y_pix = rover_coords(obstacle_map)
    ...
    obs_x_world, obs_y_world = pix_to_world(obs_x_pix, obs_y_pix,
                                            xpos, ypos,
                                            yaw, world_size, scale)
    ...
    distance, angle = to_polar_coords(xpix, ypix)
